...
$sqlMssgTu = "SELECT Tutor FROM `message` where Admin='".$_SESSION['Uname']."' and Student='' Group by Tutor";
...
?>
  ...
  <body>
	...
	while($rowMssg = mysqli_fetch_array($resultMssgTu))
			{
				$sqlMssgNew = "SELECT MssgSeen FROM `message` where IndexNum=(Select Max(IndexNum) from message where Admin='".$_SESSION['Uname']."' and Tutor='".$rowMssg['Tutor']."' and Sender ='2')";
				...
				$sqlPhnTu = "SELECT PhoneNumber FROM `reginfo` where Email='".$rowMssg['Tutor']."'";
				$resultPhnTu = $conn->query($sqlPhnTu);
				$rowPhnTu = $resultPhnTu->fetch_assoc();
				
				echo '
					<form action="adminmssgReply.php" method="post">
...
?>
	...
	<?php
		...
		{
			$sqlChtBox ="Select Message,Sender from message where Tutor='".$_SESSION['tuReply']."' and Admin='sanjida@gmail.com' Order By IndexNum DESC";
			...
			while($rowChtBox = mysqli_fetch_array($resultChtBox))
			{
				...
				if($rowChtBox['Sender']==2)
				{
					...
					echo $_SESSION['tuReply']."</font></p><br>";
		...
		$conn->query("Update message set MssgSeen='2' where Admin='".$_SESSION['Uname']."' and Student='' and Sender='2'");
