...
$sqlReq = "SELECT Tutor FROM `connection` where Connection=2 and Student='".$_SESSION['Uname']."'";
	...
	while($rowReq = mysqli_fetch_array($resultReq))
			{
				...
				$sqlPhn = "SELECT PhoneNumber FROM `reginfo` where Email='".$rowReq['Tutor']."'";
...
<?php
	while($rowMssg = mysqli_fetch_array($resultMssg))
			{
				$sqlMssgNew = "SELECT MssgSeen FROM `message` where IndexNum=(Select Max(IndexNum) from message where Student='".$_SESSION['Uname']."' and Tutor='".$rowMssg['Tutor']."')";
				$resultMssgNew = $conn->query($sqlMssgNew);
				$rowMssgNew = $resultMssgNew->fetch_assoc();
				
				if($rowMssgNew['MssgSeen']=='1')
				{
					echo '<b>'.$rowMssg['Tutor'].'</b>';
				}
				
				else
					echo $rowMssg['Tutor'];
				
				$sqlPhn = "SELECT PhoneNumber FROM `reginfo` where Email='".$rowMssg['Tutor']."'";
				$resultPhn = $conn->query($sqlPhn);
				$rowPhn = $resultPhn->fetch_assoc();
				
				echo '
					<form action="mssgReplyStu.php" method="post">
					<button type="submit" name="'.$rowPhn['PhoneNumber'].'" class="btn btn-primary">Reply</button>
					</form>
					<br>';
			}
			$conn->query("Update message set MssgSeen='2' where Student='".$_SESSION['Uname']."' and Admin='' and Sender='2'");
?>
</div>
	...
	<?php
		...
		if(isset($_SESSION['tuReply']))
		{
			$sqlChtBox ="Select Message,Sender from message where Tutor='".$_SESSION['tuReply']."' and Student='".$_SESSION['Uname']."' Order By IndexNum DESC";
			...
			while($rowChtBox = mysqli_fetch_array($resultChtBox))
			{
				...
				if($rowChtBox['Sender']==1)
				{
					...
					echo $_SESSION['Uname']."</font></p><br>";
					
				}
				
				if($rowChtBox['Sender']==2)
				{
					echo "<p align='left'><font size='5pd' color='blue'>".$_SESSION['tuReply']." : ";
