...
$sqlReq = "SELECT Student FROM `connection` where Connection=1 and Tutor='".$_SESSION['Uname']."'";
$resultReq = $conn->query($sqlReq);

$sqlCon = "SELECT Student FROM `connection` where Connection=2 and Tutor='".$_SESSION['Uname']."'";
$resultCon = $conn->query($sqlCon);


$sqlMssg = "SELECT Student FROM `message` where Tutor='".$_SESSION['Uname']."' Group by Student";
	...
	while($rowReq = mysqli_fetch_array($resultReq))
			{
				...
				$sqlPhn = "SELECT PhoneNumber FROM `reginfostu` where Email='".$rowReq['Student']."'";
				$resultPhn = $conn->query($sqlPhn);
				$rowPhn = $resultPhn->fetch_assoc();
				
				echo '
						<form action="requestAccept.php" method="post">
						<button type="submit" name="'.$rowPhn['PhoneNumber'].'" class="btn btn-primary">Accept</button>
						</form>
						<br>';
			}
?>
	...
	while($rowCon = mysqli_fetch_array($resultCon))
			{
				...
				$sqlPhn = "SELECT PhoneNumber FROM `reginfostu` where Email='".$rowCon['Student']."'";
				$resultPhn = $conn->query($sqlPhn);
				$rowPhn = $resultPhn->fetch_assoc();
				
				echo '
						<form action="disconnectTu.php" method="post">
						<button type="submit" name="'.$rowPhn['PhoneNumber'].'" class="btn btn-primary">Disconnect</button>
						</form>
						<br>';
			}
?>
...
<br><h2>Messages</h2>
<?php
	while($rowMssg = mysqli_fetch_array($resultMssg))
			{
				$sqlMssgNew = "SELECT MssgSeen FROM `message` where IndexNum=(Select Max(IndexNum) from message where Tutor='".$_SESSION['Uname']."' and Student='".$rowMssg['Student']."')";
				...
				$sqlPhn = "SELECT PhoneNumber FROM `reginfostu` where Email='".$rowMssg['Student']."'";
				$resultPhn = $conn->query($sqlPhn);
				$rowPhn = $resultPhn->fetch_assoc();
				
				echo '
					<form action="mssgReply.php" method="post">
					<button type="submit" name="'.$rowPhn['PhoneNumber'].'" class="btn btn-primary">Reply</button>
					</form>
					<br>';
			}
	$conn->query("Update message set MssgSeen='2' where Tutor='".$_SESSION['Uname']."' and Admin='' and Sender='1'");
?>
</div>
	...
	<?php
		...
		{
			$sqlChtBox ="Select Message,Sender from message where Student='".$_SESSION['stuReply']."' and Tutor='".$_SESSION['Uname']."' Order By IndexNum DESC";
			...
			while($rowChtBox = mysqli_fetch_array($resultChtBox))
			{
				...
				if($rowChtBox['Sender']==2)
				{
					echo "<p align='left'><font size='5pd' color='blue'>".$_SESSION['Uname']." : ";
				...
				echo '<b>'."No Conversaton is selected".'</b>';
