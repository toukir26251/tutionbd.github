...
?>
		...
		<?php
			...
			if(isset($_SESSION['Uname']) && $_SESSION['Uname']!=$row['Email'] && $_SESSION['Uname']!="sanjida@gmail.com" && $pass['Type']=="Tutor")
			{
				$resultCon = $conn->query("Select Connection from connection where Tutor='".$_SESSION['Uname']."' and Student='".$row['Email']."'");
				...
				if($rowCon['Connection']==2)
					{
						echo
						  '
						  <form action="firstMssgStu.php" method="post">
						  <button type="submit" name="'.$row['PhoneNumber'].'" class="btn btn-primary">" Message This Student "</button><br>
						  </form>
						  <br>';
